...
class ReasoningResponseEngine:
    ...
    def initialize_investigation_strategy(self, session_id: str, top_hypothesis, db_store):
        """Initialize the first investigation strategy based on top hypothesis."""
        strategy_name = top_hypothesis.category
        strategy_id = db_store.store_investigation_strategy(session_id, strategy_name, top_hypothesis.category)
        return strategy_name
    
    def update_strategy_metrics(self, session_id: str, strategy_name: str, 
                           evidence_signals: List[str], previous_hypotheses: List[RootCauseHypothesis],
                           current_hypotheses: List[RootCauseHypothesis], db_store):
        """Update strategy progress metrics and detect progress."""
        # Get current strategy
        strategy = db_store.get_active_strategy(session_id)
        if not strategy:
            return False
        
        # Calculate metrics
        steps_completed = strategy['steps_completed'] + 1
        evidence_events_count = strategy['evidence_events_count'] + len(evidence_signals)
        
        # Detect hypothesis changes
        hypothesis_changes = 0
        if previous_hypotheses and current_hypotheses:
            # Check if top hypothesis changed
            if previous_hypotheses[0].category != current_hypotheses[0].category:
                hypothesis_changes += 1
            
            # Check if any hypotheses were eliminated
            eliminated_count = sum(1 for h in current_hypotheses if h.eliminated)
            prev_eliminated_count = sum(1 for h in previous_hypotheses if h.eliminated)
            if eliminated_count > prev_eliminated_count:
                hypothesis_changes += 1
        
        # Check for new evidence signals
        has_progress = hypothesis_changes > 0 or len(evidence_signals) > 0
        
        # Update strategy
        db_store.update_strategy_progress(
            session_id, strategy_name,
            steps_completed=steps_completed,
            evidence_events_count=evidence_events_count,
            hypothesis_changes_count=strategy['hypothesis_changes_count'] + hypothesis_changes,
            last_progress=has_progress
        )
        
        return has_progress
    
    def select_alternative_strategy(self, session_id: str, current_strategy: str, 
                                available_hypotheses: List[RootCauseHypothesis], db_store) -> Optional[str]:
        """Select an alternative investigation strategy from different hypothesis family."""
        # Get attempted strategies
        attempted_strategies = db_store.get_attempted_strategies(session_id)
        
        # Get available hypothesis categories (non-eliminated)
        available_categories = list(set(h.category for h in available_hypotheses if not h.eliminated))
        
        # Strategy mapping based on hypothesis categories
        strategy_mapping = {
            'runtime_network': 'network_connectivity',
            'runtime_exception': 'application_debugging',
            'config_runtime': 'configuration_validation',
            'service_network': 'service_diagnostics',
            'permissions_runtime': 'permission_analysis',
            'dependency_missing': 'dependency_verification',
            'filesystem_access': 'filesystem_analysis',
            'environment_config': 'environment_variables',
            'module_import': 'module_diagnostics'
        }
        
        # Find alternative strategies
        alternative_strategies = []
        for category in available_categories:
            if category in strategy_mapping:
                strategy_name = strategy_mapping[category]
                if strategy_name not in attempted_strategies and strategy_name != current_strategy:
                    alternative_strategies.append((category, strategy_name))
        
        # Select the first alternative (deterministic selection)
        if alternative_strategies:
            return alternative_strategies[0][1]  # Return strategy name
        
        return None
    
    def generate_alternative_diagnostics(self, alternative_strategy: str, command_context: Dict[str, Any]) -> Dict[str, Any]:
        """Generate diagnostic commands for alternative investigation strategy."""
        # Alternative strategy diagnostic templates
        alternative_templates = {
            'configuration_validation': [
                {
                    'description': 'Validate configuration file syntax',
                    'command': 'python -m json.tool config.json',
                    'subsystem_specific': {
                        'yaml': 'python -c "import yaml; yaml.safe_load(open(\"config.yaml\"))"',
                        'toml': 'python -c "import toml; toml.load(open(\"config.toml\"))"'
                    }
                },
                {
            ...
            'dependency_verification': [
                {
                    ...
                    'command': 'pip list | grep -E "(requests|flask|django)"',
                    'subsystem_specific': {
                ...
                {
                    'description': 'Verify dependency versions',
                    'command': 'python -c "import pkg_resources; print(pkg_resources.get_distribution(\"requests\"))"',
                    'subsystem_specific': {}
                }
            ],
            'environment_variables': [
                {
                    ...
                    'command': 'env | grep -E "(DATABASE|API|PORT|HOST)"',
                    'subsystem_specific': {}
                },
                {
                    'description': 'Validate required environment variables',
                    'command': 'python -c "import os; print([k for k in os.environ if k in [\"DATABASE_URL\", \"API_KEY\"]])"',
                    'subsystem_specific': {}
                }
            ],
            'filesystem_analysis': [
                {
                    'description': 'Check file permissions and existence',
                    'command': 'ls -la /var/log/app.log',
                    'subsystem_specific': {}
                },
                {
            ...
            'module_diagnostics': [
                {
                    ...
                    'command': 'python -c "import requests, json, sys; print(\"All modules imported successfully\")"',
                    'subsystem_specific': {}
                },
                {
                    'description': 'Check module version compatibility',
                    'command': 'python -c "import sys; print(f\"Python version: {sys.version}\")"',
                    'subsystem_specific': {}
                }
            ],
            'service_diagnostics': [
                {
                    'description': 'Check service configuration files',
                    'command': 'systemctl cat nginx.service',
                    'subsystem_specific': {
                        'apache': 'systemctl cat apache2.service',
                        'mysql': 'systemctl cat mysql.service'
                    }
                },
                {
        ...
        # Get templates for alternative strategy
        templates = alternative_templates.get(alternative_strategy, [])
        
        if not templates:
            # Fallback to generic diagnostics
            templates = [
                {
                    'description': 'Review application logs for patterns',
                    'command': 'tail -50 /var/log/app.log | grep -E "(error|exception|timeout)"',
                    'subsystem_specific': {}
                }
            ]
        
        # Adapt to subsystem if available
        detected_subsystem = command_context.get('subsystem') if command_context else None
        primary_commands = []
        
        for template in templates[:2]:  # Limit to 2 primary commands
            command = self._adapt_command_to_subsystem(template, detected_subsystem)
            primary_commands.append({
                'description': template['description'],
                'command': command
    ...
    def _adapt_command_to_subsystem(self, template: Dict[str, Any], detected_subsystem: Optional[str]) -> str:
        """Adapt diagnostic command to detected subsystem."""
        base_command = template["command"]
