...
class DatabaseStore:
    ...
    def store_investigation_strategy(self, session_id: str, strategy_name: str, hypothesis_category: str) -> str:
        """Store a new investigation strategy for a session."""
        from uuid import uuid4
        from datetime import datetime
        ...
        conn = self.get_db_connection()
        query = """
        INSERT INTO investigation_strategies 
        (id, session_id, strategy_name, hypothesis_category, status, timestamp_started)
        VALUES (?, ?, ?, ?, ?, ?)
        """
    ...
    def update_strategy_progress(self, session_id: str, strategy_name: str, 
                             steps_completed: int = None, evidence_events_count: int = None,
                             hypothesis_changes_count: int = None, last_progress: bool = False):
        """Update strategy progress metrics."""
        from datetime import datetime
        import json
        ...
        if evidence_events_count is not None:
            updates.append("evidence_events_count = ?")
            params.append(evidence_events_count)
        
        if hypothesis_changes_count is not None:
            updates.append("hypothesis_changes_count = ?")
            params.append(hypothesis_changes_count)
        
        if last_progress:
            updates.append("last_progress_timestamp = ?")
            params.append(datetime.utcnow().isoformat())
        
        if updates:
            query = f"""
            UPDATE investigation_strategies 
            SET {', '.join(updates)}
            WHERE session_id = ? AND strategy_name = ? AND status = 'active'
            """
            
            params.extend([session_id, strategy_name])
            conn.execute(query, params)
            conn.commit()
    
    def complete_strategy(self, session_id: str, strategy_name: str):
        ...
        query = """
        UPDATE investigation_strategies 
        SET status = 'completed', timestamp_completed = ?
        WHERE session_id = ? AND strategy_name = ? AND status = 'active'
        """
        
        conn.execute(query, (datetime.utcnow().isoformat(), session_id, strategy_name))
        conn.commit()
    
    def get_active_strategy(self, session_id: str) -> Optional[Dict[str, Any]]:
        """Get the currently active investigation strategy."""
        conn = self.get_db_connection()
        query = """
        SELECT id, strategy_name, hypothesis_category, status, timestamp_started,
               steps_completed, evidence_events_count, hypothesis_changes_count, last_progress_timestamp
        FROM investigation_strategies 
        ...
        if result:
            return {
                ...
                'hypothesis_category': result['hypothesis_category'],
                'status': result['status'],
                'timestamp_started': result['timestamp_started'],
                'steps_completed': result['steps_completed'],
                'evidence_events_count': result['evidence_events_count'],
                'hypothesis_changes_count': result['hypothesis_changes_count'],
                'last_progress_timestamp': result['last_progress_timestamp']
            }
        
        return None
    
    def get_attempted_strategies(self, session_id: str) -> List[str]:
        """Get list of strategies already attempted for a session."""
        conn = self.get_db_connection()
        query = """
        ...
        results = conn.execute(query, (session_id,)).fetchall()
        return [result['strategy_name'] for result in results]
    
    def detect_investigation_stall(self, session_id: str) -> Optional[Dict[str, Any]]:
        """Detect if investigation has stalled based on deterministic rules."""
        conn = self.get_db_connection()
        
        # Get active strategy
        strategy = self.get_active_strategy(session_id)
        if not strategy:
            return None
        
        # Check stall conditions
        steps_completed = strategy['steps_completed']
        hypothesis_changes = strategy['hypothesis_changes_count']
        
        # Stall detection: 2+ steps completed AND no hypothesis changes
        if steps_completed >= 2 and hypothesis_changes == 0:
            # Check for repeated evidence patterns
            evidence_query = """
            SELECT evidence_generated 
            FROM investigation_steps 
            ...
            evidence_results = conn.execute(evidence_query, (session_id,)).fetchall()
            
            # Check if evidence patterns are repeating
            if len(evidence_results) >= 2:
                evidence_sets = []
                for result in evidence_results:
                    import json
                    evidence = json.loads(result['evidence_generated'] or '[]')
                    evidence_sets.append(set(evidence))
                
                # Check for high overlap between recent evidence sets
                if len(evidence_sets) >= 2:
                    overlap_ratio = len(evidence_sets[0] & evidence_sets[1]) / max(len(evidence_sets[0] | evidence_sets[1]), 1)
                    if overlap_ratio >= 0.7:  # 70% overlap indicates stall
                        return {
                            'stalled': True,
                            'reason': 'Multiple diagnostic steps produced overlapping evidence without narrowing hypotheses',
                            'current_strategy': strategy['strategy_name'],
                            'steps_completed': steps_completed,
                            'evidence_overlap': overlap_ratio
                        }
